    ...
    <div className="relative w-full flex flex-col items-center p-10">
      <div className="relative w-full max-w-5xl flex items-center justify-between mt-10">
        <motion.img
          src="/auto.png"
          alt="Auto-Rickshaw"
          className="w-32 h-32 absolute -top-25"
          animate={{ left: `${autoPosition}%` }}
          transition={{ ease: "linear", duration: 2, repeat: Infinity }}
        />

        <div className="w-full flex justify-between items-center border-t-4 border-yellow-600/40 relative">
          {events.map((event, index) => (
            <div key={index} className="relative flex flex-col items-center cursor-pointer space-y-4">
              {/* Timeline Dot */}
              <div className="w-6 h-6 bg-yellow-600 rounded-full mt-[-12px]"></div>
              <span className="text-sm mt-2">{event.time}</span>
              ...
              >
                ...
                >
                  ...
                  <div className="absolute w-full h-full bg-red-900  rounded-xl shadow-lg p-4 flex flex-col justify-center items-center backface-hidden"
                    ...
                    <h2 className="text-white text-md font-bold font-imenglish">{event.title}</h2>
                    <p className="text-gray-200 mt-2 text-sm font-imenglish">{event.description}</p>
                  </div>
                </motion.div>
              </motion.div>
            </div>
